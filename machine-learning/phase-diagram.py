...
#x = iris.data[:, [2, 3]]
#y = iris.target
df = pd.read_csv('tnr-3-1.data')
phase_mapping = {'Disordered':0, 'Lamellae':1,'Nematic-lamellae':2,'Bilayer-arrowhead':3,'Hexatic':4,'Hexagonal-cylinder':5}
df['phase'] = df['phase'].map(phase_mapping)

x = df.iloc[0:145, [0, 1]].values
y = df.iloc[0:145, 2].values
print np.unique(y)
...
plot_decision_regions(x=x_combined_std, y=y_combined, classifier=tree, test_idx=range(98,140))
plt.xlabel('tether fraction [standardized]')
plt.ylabel('1/T [standardized]')
